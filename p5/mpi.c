...
{
    MPI_User_function *fun=funptr;
    MPI_Status stat;
    int i, j, done = 0;
    int elesize;
    char *tempbuf = (char*)malloc(sizeof(char)*10);
    int *temprbuf = (int*)malloc(sizeof(int)*size*count);
    int *tempint = (int*)malloc(sizeof(int)*count);
    int test;
    int test2[count];
    MPI_Datatype *typetest;
    int *add;
    typetest=&type;

    for (i=0;i<count;i++) test2[i]=0;
    if (type==MPI_INT) elesize=sizeof(int);
    ...
    if(op==MPI_SUM)
    {
        if (rank==root)
        {
            for (i=0;i<size;i++)
            {
                if (i!=rank)
                {
                    MPI_Recv(tempint,count,type,i,0,comm,&stat);
                    for (j=0;j<count;j++)
                    {
                        memcpy(&test,tempint+j,sizeof(int));
                        memcpy(temprbuf+(stat.MPI_SOURCE*count)+j,&test,sizeof(int));
                    }
                }
                else
                {
                    for (j=0;j<count;j++)
                    {
                        memcpy(&test,sendbuf+j*sizeof(int),sizeof(int));
                        memcpy(temprbuf+(rank*count)+j,&test,sizeof(int));
                    }
                }
            }
            for(i=0;i<size;i++)
                if(i!=rank)
                    write(clnt2serv[i],"ACK",sizeof(char)*4);
            for (i=0;i<size;i++)
            {
                memcpy(&test,temprbuf+(count*i)+j,sizeof(int)-1);
                test2[i]+=test;
            }
            for (i=0;i<count;i++)
                memcpy(recvbuf+i*sizeof(int),&test2[i],sizeof(int));
        }
        else
        {
            MPI_Send(sendbuf,count,type,root,0,comm);
            read(clnt2serv[root],tempbuf,sizeof(char)*0);
        }
    }
    else
    {
        if (rank==root)
        {
            bzero(recvbuf,sizeof(int)*count);
            for(i=0;i<size;i++)
            {
                if(i!=rank)
                {
                    ...
                    {
                        memcpy(&test,tempint+j,sizeof(int));
                        memcpy(temprbuf+j,&test,sizeof(int));
                    }
                    (*fun)(temprbuf,recvbuf,&count,typetest);
                    for(j=0;j<count;j++)
                        memcpy(&test,recvbuf+j*sizeof(int),sizeof(int));
                }
                else
                {
                    for(j=0;j<count;j++)
                    {
                        memcpy(&test,sendbuf+j*sizeof(int),sizeof(int));
                        memcpy(recvbuf+j*sizeof(int),&test,sizeof(int));
                    }
                }
            }
            for(i=0;i<size;i++)
        ...
        {
            MPI_Send(sendbuf,count,type,root,0,comm);
            read(clnt2serv[root],tempbuf,sizeof(char)*4);
        }
    }
...
int MPI_Op_create(MPI_User_function *fp, int unused, MPI_Op *op)
{
    funptr=fp;
}

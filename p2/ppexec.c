...
int main (int argc, char * argv[])
{
	...
	FILE *f;
	char *rank="; cd $PWD; PP_MPI_RANK=",*size = "PP_MPI_SIZE=";
	char *port1="PP_MPI_HOST_PORT=",*port2 = ":4401";
	char *ssh="ssh",*arg1="bash -c 'PWD=",*end="'",*space=" ";
	char *hostArr[99],*hostFile,*sizeStr,*ppSize,*rankStr,*ppRank,cwdPath[1024];
	char *ppPort,*sshHost,*sshArg,*curPath,bashCmd[100],hostName[1024];
	bashCmd[100]='\0';
	hostName[1024]='\0';
	...
	if (argv[argc-1][0] == '.')
	{
		char * temp = malloc ((strlen(argv[argc-1]) - 2) * sizeof(char));
		int q=0;
		while (q+2<strlen(argv[argc-1]))
		{
			temp[q] = argv[argc-1][q+2];
			q++;
		}
		printf("temp: %s\n",temp);
		argv[argc-1] = temp;
	...
	while(j<n)
	{
		...
		sshArgLen = strlen(arg1)+strlen(curPath)+strlen(ppRank)+strlen(ppPort)+
					strlen(ppSize)+strlen(bashCmd)+strlen(end);
		sshArg = malloc((sshArgLen)*sizeof(char));
		strcpy(sshArg,arg1); strcat(sshArg,curPath); strcat(sshArg,ppRank); strcat(sshArg,ppSize);
